...
const searchedValue = searchArea.value.toLowerCase().trim();

  const filteredProducts = products.filter(product => {
    return product.title.toLowerCase().includes(searchedValue);
  });
...
function showProductsList(filteredProducts) {
productsList.innerHTML='';

for(const pro of filteredProducts) {

  productsList.innerHTML+=
  `<li class='product-card'>
    <img src="${pro.thumbnail}" alt="Product Image">
    <h2> title :  </h2>${pro.title}
    <h2> brand :  </h2> ${pro.brand}
    <h2> category : </h2> ${pro.category}
    <h2> description :  </h2> <p> ${pro.description}</p>
    <h2> price : </h2> ${pro.price}
    <h2> stock : </h2> ${pro.stock}
    <h2> tags : </h2>  ${pro.tags}
  </li>`
  }
